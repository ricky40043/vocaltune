...
song = replace_once(song, "        if (!isActive) return;", "        if (!isActive || mode !== 'queue') return;", "queue polling guard")
song = replace_once(song, "    }, [isActive, userQuery]);", "    }, [isActive, userQuery, mode]);", "queue polling deps")
song = replace_once(
    song,
...
song = replace_once(song, "onClick={() => addToQueue(video)}", "onClick={() => mode === 'select' ? onSelectSong?.(video) : addToQueue(video)}", "select handler")
song = replace_once(
    song,
    "<Plus size={18} />\n                                        加入點歌",
...
app = replace_once(app, state, state + "\n    const [selectedSourceTitle, setSelectedSourceTitle] = useState<string | null>(null);", "selected source title state")
...
app = replace_once(app, ">v4.0<", ">v4.0.1<", "display version")
