...
await evaluate(`[...document.querySelectorAll('button')].find(el => el.textContent.includes('變調器'))?.click()`);
await new Promise(resolve => setTimeout(resolve, 800));
const pitcherResult = await evaluate(`JSON.stringify({ready:document.body.innerText.includes('準備就緒'), sourceRequested:performance.getEntriesByType('resource').some(entry => entry.name.includes('/files/downloads/source.m4a'))})`);
const pitcher = JSON.parse(pitcherResult.result.value);
if (!pitcher.ready || !pitcher.sourceRequested) throw new Error(`History pitcher source verification failed: ${JSON.stringify(pitcher)}`);
...
await new Promise(resolve => setTimeout(resolve, 500));
const mobileReadyResult = await evaluate(`JSON.stringify({playable:Boolean(document.querySelector('button[aria-label="播放分離音軌"]:not([disabled])')), prompt:document.body.innerText.includes('點擊開始載入與播放'), audioCount:document.querySelectorAll('audio[data-separation-track]').length})`);
const mobileReady = JSON.parse(mobileReadyResult.result.value);
if (!mobileReady.playable || !mobileReady.prompt || mobileReady.audioCount !== 6) throw new Error(`Mobile playback readiness verification failed: ${JSON.stringify(mobileReady)}`);
const screenshot = await send('Page.captureScreenshot', { format: 'png', captureBeyondViewport: false });
await fs.writeFile('/tmp/vocaltune-history-six-tracks.png', Buffer.from(screenshot.data, 'base64'));
console.log(JSON.stringify({desktop: state, pitcher, mobile: mobileState, mobileReady}));
socket.close();
