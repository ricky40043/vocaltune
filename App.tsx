            ...
            <main className="flex-1 max-w-md mx-auto w-full px-4 py-6">
                ...
                <div style={{ display: activeTab === 'source' ? 'block' : 'none' }} className="space-y-6 animate-fade-in">
                    ...
                    <div className="border-t border-gray-700/50 pt-6">
                        <h3 className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-3 px-1">或上傳本地檔案</h3>
                        <label className="block p-6 rounded-2xl border-2 border-dashed border-gray-600 text-center hover:border-brand-accent transition-colors cursor-pointer bg-gray-800/30 hover:bg-gray-800/50">
                            <input
                                type="file"
                                accept="audio/*"
                                className="hidden"
                                onChange={(e) => {
                                    if (e.target.files?.[0]) {
                                        ...
                                        // TODO: Need state for local file URL
                                        // For now we rely on LocalPlayer notifying us, or we set it here if we lift state
                                        // Since we are creating ObjectURL here, we updates App state directly
                                        // But current implementation relies on `downloadedFileUrl`.
                                        // Let's reuse `downloadedFileUrl` variable to mean "current active URL" or add a new state.
                                        // Adding new state is cleaner but requires prop updates.
                                        // For speed, let's treat `downloadedFileUrl` as "Current Global URL"
                            ...
                            <p className="text-gray-400 font-medium">匯入音樂(MP3/WAV)</p>
                            <p className="text-xs text-gray-600 mt-1">點擊上傳後進入變調器</p>
                        </label>
                    </div>
                </div>

                {/* TAB 2: PITCHER - 變調器 */}
                <div style={{ display: activeTab === 'pitcher' ? 'block' : 'none' }} className="animate-fade-in">
                    <LocalPlayer
                ...
                {/* TAB 3: SPLITTER - 分離器 */}
                <div style={{ display: activeTab === 'splitter' ? 'block' : 'none' }} className="animate-fade-in space-y-4">
                    <LocalAISeparator
                        audioFileUrl={downloadedFileUrl ? (downloadedFileUrl.startsWith('blob:') ? downloadedFileUrl : `${API_BASE_URL}${downloadedFileUrl}`) : undefined}
                    />
                </div>
